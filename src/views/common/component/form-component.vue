<template>
  <el-form ref="dataForm" :label-width="width" :model="dataForm" :rules="dataRule">
    <el-form-item v-for="(item,index) in formItemList" :key="index" :label="item.name" :prop="item.title">
      ...
      <div class="form-input-item">
        ...
        <!-- 开关输入项 -->
        <el-switch v-else-if="item.type == 'switch'" v-model="dataForm[item.title]" active-value="1" inactive-value="2" :active-text="item.params.active" :inactive-text="item.params.inactive" @input="handleValue">
        </el-switch>
  ...
  export default {
    ...
    methods: {
      init ()
      {
        this.$nextTick(() => {
          this.$http({
            url: this.$http.adornUrl(this.urlInit),
            method: 'get',
      ...
      dataFormSubmit () {
        this.$http({
          url: this.$http.adornUrl(this.urlSubmit),
          method: 'post',
