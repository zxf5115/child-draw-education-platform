    ...
    <div class="admin_main_block">
      <div class="admin_main_block_top">
        <div class="admin_main_block_left">
          <div>
            <el-button v-if="isAuth('module:education:course:intensify:form')" type="success" icon="el-icon-plus" @click="$router.push({name: 'module_education_course_intensify_form'})">
              {{ $t('common.create') }}
            </el-button>
          </div>
          <div>
            <el-button v-if="isAuth('module:education:course:intensify:question:list')" icon="el-icon-price-tag" @click="$router.push({name: 'module_education_course_intensify_question_list'})">
      ...
      <div class="admin_table_main">
        <el-table :data="dataList" v-loading="dataListLoading" @selection-change="selectionChangeHandle">
          ...
          <el-table-column :label="$t('common.handle')" fixed="right" width="200">
            <template slot-scope="scope">
              <el-button v-if="isAuth('module:education:course:intensify:form')" type="primary" icon="el-icon-edit" @click="$router.push({name: 'module_education_course_intensify_form', query: {id: scope.row.id, type: scope.row.type.value}})">
                {{ $t('common.update') }}
              </el-button>
