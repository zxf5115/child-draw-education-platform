<template>
  <el-form label-width="100px" ref="dataForm" :model="dataForm" :rules="dataRule">
    ...
    <el-form-item :label="$t('paper.title')" prop="paper_id">
      <el-select v-model="dataForm.paper_id" multiple collapse-tags filterable clearable>
        <el-option v-for="(v,k) in paperList" :label="v.title" :key="k" :value="v.id">
        </el-option>
      </el-select>
    </el-form-item>
...
<script>
  ...
  export default
  {
    ...
    methods:
    {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              this.$http({
                url: this.$http.adornUrl(`/education/course/intensify/paper/${this.dataForm.id}`),
                method: 'get',
                params: this.$http.adornParams()
              }).then(({data}) => {
                if (data && data.status === 200) {
                  this.dataForm.paper_id = data.data
                }
              })
            })
          }
      ...
      loadPaperList () {
        this.$http({
          url: this.$http.adornUrl('/education/paper/select'),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {
          if (data && data.status === 200) {
